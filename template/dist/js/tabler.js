...
(function (factory) {
	...
	function reload() {
	  ...
	  maskElementList.map(function (maskEl) {
	    if (!maskEl.dataset['mask_id']) {
	      var id = v4();
	      maskEl.setAttribute('data-mask_id', id);
	    }
	    var tmp = new IMask(maskEl, {
	      mask: maskEl.dataset.mask,
	      lazy: maskEl.dataset['mask_visible'] === 'true'
	    });
	    if (typeof IMaskEl[maskEl.dataset['mask_id']] === 'undefined') IMaskEl[maskEl.dataset['mask_id']] = tmp;
	    return tmp;
	  });
	}
	function get(maskEl) {
	  if (maskEl.dataset['mask_id']) return IMaskEl[maskEl.dataset['mask_id']];
	  return false;
	}
	window.IMaskEl = {
	  get: get,
